...
def concat_videos(delete_images: bool = True):
    ...
    audio_clips = list(map(AudioFileClip, map(str, map(Config.output_audios.joinpath, map(Config.audio_name_normalization, PassedVariables.texts_to_translate)))))
    ...
    for video_clip, pause_frame, audio_clip in zip_longest(video_clips, islice(chain.from_iterable((pause_frames, repeated_frame)), len(audio_clips)), audio_clips):
